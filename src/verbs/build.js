...
var _build = function (src, dst, opts) {
  ...
  const sheetObjects = ResumeFactory.load(
    src,
    {
      format: null,
      objectify: false,
  ...
  if (sheets.length > 1) {
    ...
    const mixed = _.any(sheets, function (s) {
      if (isFRESH) {
        return s.basics;
      } else {
        return !s.basics;
...
var _prep = function (src, dst, opts) {
  ...
  _opts.theme =
    (opts.theme && opts.theme.toLowerCase().trim()) ||
    "node_modules/jsonresume-theme-hexagon";
...
var _expand = function (dst, theTheme) {
  ...
  const destColl = (dst && dst.length && dst) || [
...
var _loadTheme = function (tFolder) {
  ...
  const theTheme = exists(themeJsonPath)
