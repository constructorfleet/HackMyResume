...
module.exports = function (rawArgs, exitCallback) {
  const initInfo = initialize(rawArgs, (...args) =>
  ...
  program
    ...
    .option(
      "-f --format <fmt>",
      "FRESH or JRS format and optional version",
      undefined,
    )
    .action(function () {
...
var initialize = function (ar, exitCallback) {
  ...
  if (o.ex) {
    ...
    if (o.ex.op === "parse") {
      _err.err({
        fluenterror:
          o.ex.op === "parse"
            ? HMSTATUS.invalidOptionsFile
            : HMSTATUS.optionsFileNotFound,
        inner: o.ex.inner,
        quit: true,
      });
    } else {
  ...
  if (o.debug) {
    _out.log(
      chalk.cyan("The -d or --debug switch was specified. DEBUG mode engaged."),
    );
    _out.log("");
    _out.log(
      chalk.cyan(PAD("  Platform:", 25, null, PAD.RIGHT)) +
        chalk.cyan.bold(
          process.platform === "win32" ? "windows" : process.platform,
        ),
    );
    _out.log(
      chalk.cyan(PAD("  Node.js:", 25, null, PAD.RIGHT)) +
        chalk.cyan.bold(process.version),
    );
    _out.log(
      chalk.cyan(PAD("  HackMyResume:", 25, null, PAD.RIGHT)) +
        chalk.cyan.bold(`v${PKG.version}`),
    );
    _out.log(
      chalk.cyan(PAD("  FRESCA:", 25, null, PAD.RIGHT)) +
        chalk.cyan.bold(PKG.dependencies.fresca),
    );
  ...
  Command.prototype.helpInformation = function () {
    const manPage = FS.readFileSync(
      PATH.join(__dirname, "help/use.txt"),
      "utf8",
    );
...
var initOptions = function (ar) {
  ...
  if (cleanArgs.length) {
    ...
    const optsIdx = _.findIndex(
      cleanArgs,
      (v) => v === "-o" || v === "--options" || v === "--opts",
    );
...
var loadOptions = function (o, cmdO) {
  ...
  if (o.debug) {
    ...
    _.each(o, (val, key) =>
      logMsg(
        chalk.cyan("  %s") + chalk.cyan.bold(" %s"),
        PAD(key, 22, null, PAD.RIGHT),
        val,
      ),
    );
    logMsg("");
